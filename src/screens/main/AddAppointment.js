...
class LoginScreen extends Component {
    ...
    async CreateApp() {
        ...
        if (credentials) {
            ...
            const {data} = await Request.CreateAppointment(this.state, auth.accessToken).then((response) => {
                return response;
            })
            console.log(data)
            if (data.status) {
    ...
    handleLoginPress = async () => {
        ...
        if (validator) {
            ...
            Alert.alert(
                'Are You Sure?',
                'Are you sure create Appointment?',
                [
                    {
                        text: 'Cancel',
    ...
    handleOnChange = (date) => {
        ...
        this.setState({
            deadline: new Date(moment(date).format('MM-DD-YYYY') + " " + showtime),
    ...
    handleGetTime = (date) => {
        const {showdate, showtime} = this.state;
        this.setState({
            deadline: new Date(showdate + " " + moment(date).format('hh:mm A')),
            showtime: moment(date).format('hh:mm A'),
            opentime: false
        })
    }
    ...
    getUsers = async () => {
        ...
        if (credentials) {
            ...
            this.setState({doctor: doctor, user: user, loading: false, auth: auth})
        } else {
            this.props.navigation.navigate(Screens.auth.LoginScreen)
        }
    ...
    render() {
        const {
            error,
            title, description, loading, showdate, showtime, image
        } = this.state;
        console.log(new Date(showdate + " " + showtime), "mydate==>", this.state)
        return (
                    ...
                    <View style={styles.container}>
                        ...
                        <View style={styles.SectionStyle}>
                            ...
                            <DateTimePicker
                                ...
                                onCancel={this.handleCloseDialog}
                                onConfirm={(date) => {
                                    this.handleGetTime(date)
                                }}
                            />
                        </View>
                        <Text
                            style={[styles.error, {display: this.state.role == 'ROLE_USER' ? 'none' : ''}]}>{error && error.showtime ? error.showtime[0] : ''}</Text>
                        ...
                        <TouchableOpacity onPress={this.handleUpload}>
                            <Image source={image != '' ? {uri: "data:image/jpg;base64," + image} : Assets.upload}
                                   style={styles.logo}/>
                        </TouchableOpacity>
